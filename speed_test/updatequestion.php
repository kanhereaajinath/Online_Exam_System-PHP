...
<body>
    ...
    nextButton.addEventListener("click", function () {
        ...
        let numQuestions1 = <?php
// Connect to your database
include_once("dbConnection.php");

// Initialize numQuestions1 variable
$numQuestions1 = 0;

// Get the exam ID from the POST data, assuming it's submitted via a form
if(isset($_GET['eid'])) {
    $examid = mysqli_real_escape_string($con, $_GET['eid']);

    // Query to get the maximum serial number
    $query = "SELECT MAX(sn) as max_sn FROM `questions` WHERE eid='$examid'";
    $result = mysqli_query($con, $query) or die(mysqli_error($con));
    $row = mysqli_fetch_assoc($result);
    $max_sn = $row['max_sn'];

    // Store the maximum serial number in $numQuestions1 as an integer
    $numQuestions1 = (int)$max_sn;
}

// Close the connection
mysqli_close($con);

// Output the value of numQuestions1 as an integer
echo $numQuestions1;
?>

numQuestions1 += 1;
        // Create the form with the specified number of questions
        let formHTML = '<form id="form2" class="form-horizontal title1" name="form" action="insertquestion.php?q=addqns&n=' + numQuestions1 + '&eid=' + encodeURIComponent(window.location.href.split("?")[1].split("&")[0].split("=")[1]) + '&ch=4&qn=' + numQuestions + ' "  method="POST"><fieldset>';
        ...
        formHTML += '<div class="form-group"><label class="col-md-12 control-label" for=""></label><div class="col-md-12"><input  type="submit" style="margin-left:54%" class="btn btn-success" value="Submit" class="btn btn-primary"/><div class="demo"></div></div></div></fieldset></form>';
...
<!-- Example form to submit exam ID -->
